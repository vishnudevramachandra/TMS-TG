...
def plot(tms, activeNeu, colParams=None, xlim=None):
    ...
    for colIdx in range(len(colParams)):
        ...
        if any(selectBlocksinfoIdx):
            ...
            plot_populationAvgFR(tms, ps_T_corrected, ps_FR,
                                 ...
                                 colIdx, colorsPlt, [item['selectionParams']['MT'] for item in traceConds])
            if colIdx == 0:
            ...
            ax[1][colIdx].set_xticks([1, 2, 3],
                                     labels=[item['selectionParams']['MT'] for item in traceConds])
    ...
    plt.show()
    plt.savefig('figure2.eps', dpi='figure', format='eps')
